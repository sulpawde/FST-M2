-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/sulbha/Input/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/sulbha/Input/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/sulbha/Input/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/sulbha/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/sulbha/outputs' USING PigStorage('\t');
